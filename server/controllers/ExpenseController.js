...
module.exports.saveUserExpense = async (req, res) => {
    ...
    for (const userPair of oweList) {
        ...
        const userResult = await ConnectionModel.findOne({
            ...
            console.log(
                'Exception in saveUserExpense ConnectionModel.findOne : '
            )
            console.log(err)
        ...
        if (userResult != null) {
            //connection exists
            if (userPair.user1Id == userResult.user1Id) {
                //user1 in connection is same as userpair
                console.log(
                    userResult.balance,
                    userPair.owes,
                    userResult.balance + userPair.owes
                )
                await ConnectionModel.findByIdAndUpdate(userResult._id, {
                    balance: userResult.balance + userPair.owes,
                }).catch((err) => {
                    console.log(
                        'Exception in saveUserExpense ConnectionModel.findByIdAndUpdate-1 : '
                    )
                    console.log(err)
                    res.status(500).send(err)
                    return
                ...
                await ConnectionModel.findByIdAndUpdate(userResult._id, {
                    ...
                    console.log(
                        'Exception in saveUserExpense ConnectionModel.findByIdAndUpdate-2 : '
                    )
                    console.log(err)
                    res.status(500).send(err)
                    return
            ...
            //connection does not exist, create connection
            ...
            await ConnectionModel.create({
                ...
                console.log(
                    'Exception in saveUserExpense ConnectionModel.create : '
                )
                console.log(err)
                res.status(500).send(err)
                return
        ...
        await UserModel.findByIdAndUpdate(userPair.user1Id, {
            totalOwes: -userPair.owes,
        }).catch((err) => {
            console.log(
                'Exception in saveUserExpense UserModel.findByIdAndUpdate-1 : '
            )
            console.log(err)
            res.status(500).send(err)
            return
        ...
        await UserModel.findByIdAndUpdate(userPair.user2Id, {
            totalOwes: userPair.owes,
        }).catch((err) => {
            console.log(
                'Exception in saveUserExpense UserModel.findByIdAndUpdate-1 : '
    ...
    await ExpenseModel.create({ ...req.body, finalShare, oweList })
        .then((result) => {
...
module.exports.updateUserExpense = async (req, res) => {
    ...
    if (keys.length == 1 && body?.title) {
        ExpenseModel.findByIdAndUpdate(id, { ...body })
            .then((result) => {
